...
server.get('/api/v1/goods', async (req, res) => {
  const data = await readFile(urlData, { encoding: 'utf8' }).then((text) => JSON.parse(text))
...
server.get('/api/v1/rates', async (req, res) => {
  const rate = await axios.get('https://api.exchangerate.host/latest?base=USD&symbols=USD,EUR,CAD')
